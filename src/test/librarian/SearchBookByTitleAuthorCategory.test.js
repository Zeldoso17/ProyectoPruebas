...
describe('Search book by title', () => {
    ...
    it("must validate if doesn't exist a book with that title", async () => {
        console.log('--------------------------------------------------------------------------------------------');
        console.log('-             VALIDAR SI NO EXISTE UN LIBRO CON ESE TITULO, AUTOR Y CATEGORIA              -');
        console.log('--------------------------------------------------------------------------------------------');

        const book = await Book.find({title: 'La vida es bella', author: 'Jose Lopez', category: 'Romance'})
        if(book.length === 0) console.log('No se encontró ninguna concidencia')
